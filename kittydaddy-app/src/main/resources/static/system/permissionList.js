        ...
        $.post(queryUrl, queryParam, function (data) {
    		...
    		laypage({
    			...
    			jump : function(obj,first){
    				$(".permission_content").html(renderData(data));
	...
	function renderData(data){
		...
		if(currData.length != 0){
			for(var i=0;i<currData.length;i++){
				var moduleName='',moduleCode='',permissionUrl='',permissionType='',parentName = '',createTime='';
				if(currData[i].moduleName != null){
					moduleName = currData[i].moduleName;
				}
				
				if(currData[i].moduleCode != null){
					moduleCode = currData[i].moduleCode;
				}
				
				if(currData[i].permissionUrl != null){
					permissionUrl = currData[i].permissionUrl;
				}
				
				if(currData[i].permissionType !=null ){
					if(currData[i].permissionType == 0){
						permissionType = '目录';
					}else{
						permissionType = '资源';
					}
				...
				if(currData[i].parentName !=null){
					parentName = currData[i].parentName;
				}
